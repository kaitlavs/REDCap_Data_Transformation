...
checkbox_df = return_new_row_df(meta, 'field type', 'checkbox', 'label', p_m_matches)
...
c_patient_list = checkbox_df['label'].tolist()
...
check_options = return_all_choices(c_patient_list, checkbox_df)
...
# 'checklist'
counter4 = 0
while counter4 < len(c_patient_list):
    c_option = slice_list(check_options, counter4)
    # metadata_df list ready to compare
    c_parsed_option = parse_choices(c_option)
    c_value = c_patient_list[counter4]
    clist_patient1 = new_patient_df[c_value].tolist()
    for i in len(clist_patient1):
